...
[[v4.3.8]]
== Version 4.3.8 (15 Sep 2026)

https://packages.couchbase.com/clients/kafka/4.3.8/couchbase-kafka-connect-couchbase-4.3.8.zip[Download]

This maintenance release includes a new SchemalessJsonConverter and some dependency updates.

=== Enhancements

* https://jira.issues.couchbase.com/browse/KAFKAC-431[KAFKAC-431]:
Add ability to specify cipher suites for the connection to Couchbase clusters.
In Source Connectors this applies to both the DCP and Client connections.

* https://jira.issues.couchbase.com/browse/KAFKAC-432[KAFKAC-432]:
Upgraded Couchbase java client to `3.12.2`

* https://jira.issues.couchbase.com/browse/KAFKAC-433[KAFKAC-433]:
Upgraded DCP client from `0.57.0` to `0.58.0`

[[v4.3.7]]
== Version 4.3.7 (19 Aug 2026)

https://packages.couchbase.com/clients/kafka/4.3.7/couchbase-kafka-connect-couchbase-4.3.7.zip[Download]

This maintenance release includes a new SchemalessJsonConverter and some dependency updates.

=== Enhancements

* https://jira.issues.couchbase.com/browse/KAFKAC-427[KAFKAC-427]:
The new SchemalessJsonConverter is designed to work interchangeably with the JsonConverter without all the schema code under the hood.
It is now used internally instead of the JsonConverter for certain parts of the Sink Connector.

* https://jira.issues.couchbase.com/browse/KAFKAC-428[KAFKAC-428]:
Upgraded Jackson from `2.20.1` to `2.21.5`

* https://jira.issues.couchbase.com/browse/KAFKAC-429[KAFKAC-429]:
Upgraded Micrometer from `1.10.4` to `1.15.12`

* https://jira.issues.couchbase.com/browse/KAFKAC-430[KAFKAC-430]:
Upgraded Jsoup from `1.21.2` to `1.23.1`

[[v4.3.6]]
== Version 4.3.6 (26 May 2026)

https://packages.couchbase.com/clients/kafka/4.3.6/couchbase-kafka-connect-couchbase-4.3.6.zip[Download]

This maintenance release includes a dependency upgrade for bugfixes.

=== Enhancements

* https://jira.issues.couchbase.com/browse/KAFKAC-425[KAFKAC-425]:
Upgraded Couchbase Java SDK from `3.11.1` to `3.11.3`

[[v4.3.5]]
== Version 4.3.5 (19 May 2026)

https://packages.couchbase.com/clients/kafka/4.3.5/couchbase-kafka-connect-couchbase-4.3.5.zip[Download]

This maintenance release includes a few minor improvements.

=== Enhancements

* https://jira.issues.couchbase.com/browse/KAFKAC-423[KAFKAC-423]:
*Source*
The DeserializeJson transform now gives the record a boolean header 'couchbase.value.is.json.object' if the root of the document is a JSON Object

[[v4.3.4]]
== Version 4.3.4 (20 Mar 2026)

https://packages.couchbase.com/clients/kafka/4.3.4/couchbase-kafka-connect-couchbase-4.3.4.zip[Download]

This maintenance release includes a few minor improvements.

=== Enhancements

* https://jira.issues.couchbase.com/browse/KAFKAC-418[KAFKAC-418]:
*Sink*
Make documentation clearer for subdocument handler create path config option

* https://jira.issues.couchbase.com/browse/KAFKAC-420[KAFKAC-420]:
Upgraded Couchbase Java SDK from `3.10.1` to `3.11.1`

* https://jira.issues.couchbase.com/browse/KAFKAC-421[KAFKAC-421]:
Upgraded DCP client from `0.56.0` to `0.57.0`

=== Bug Fixes

* https://jira.issues.couchbase.com/browse/KAFKAC-419[KAFKAC-419]:
*Source*
DeserializeJson transform now correctly ignores JSON primitives and Arrays

[[v4.3.3]]
== Version 4.3.3 (20 Jan 2026)

https://packages.couchbase.com/clients/kafka/4.3.3/couchbase-kafka-connect-couchbase-4.3.3.zip[Download]

This release adds experimental support for Schema Registry enforcement, and upgrades dependency versions.

=== Enhancements

* https://jira.issues.couchbase.com/browse/KAFKAC-415[KAFKAC-415]:
*Source*
Makes InvalidPathExceptions from JSONPath filtering fail the filter rather than be propagated.

* https://jira.issues.couchbase.com/browse/KAFKAC-416[KAFKAC-416]:
*Source*
Add more user control over the behaviour when there is a missing schema or schema mismatch.

* https://jira.issues.couchbase.com/browse/KAFKAC-417[KAFKAC-417]:
Upgraded Couchbase Java SDK from `3.10.0` to `3.10.1`

=== Bug Fixes

* https://jira.issues.couchbase.com/browse/KAFKAC-412[KAFKAC-412]:
*Source*
JSONPath filters now correctly ignore non-json documents.

[[v4.3.2]]
== Version 4.3.2 (8 Dec 2025)

https://packages.couchbase.com/clients/kafka/4.3.2/couchbase-kafka-connect-couchbase-4.3.2.zip[Download]

This release adds experimental support for Schema Registry enforcement, and upgrades dependency versions.

=== Enhancements

* https://jira.issues.couchbase.com/browse/KAFKAC-408[KAFKAC-408]:
*Source*
Add experimental support for schema enforcement for the source connector using Avro and the Confluent Schema Registry.

* https://jira.issues.couchbase.com/browse/KAFKAC-413[KAFKAC-413]:
Upgraded Couchbase Java SDK from `3.9.2` to `3.10.0`
...
[[v4.3.1]]
== Version 4.3.1 (22 Oct 2025)

https://packages.couchbase.com/clients/kafka/4.3.1/couchbase-kafka-connect-couchbase-4.3.1.zip[Download]

This release adds experimental support for config-driven schemas, and upgrades dependency versions.

=== Enhancements

* https://jira.issues.couchbase.com/browse/KAFKAC-399[KAFKAC-399]:
*Source*
Added experimental support for config-driven schemas via xref:source-configuration-options.adoc#couchbase.value.schema[the new `couchbase.value.schema` connector config property].

* https://jira.issues.couchbase.com/browse/KAFKAC-404[KAFKAC-404]:
Upgraded Couchbase Java SDK from `3.9.0` to `3.9.2`

* https://jira.issues.couchbase.com/browse/KAFKAC-409[KAFKAC-409]:
Upgraded DCP client from `0.55.0` to `0.56.0`

[[v4.3.0]]
== Version 4.3.0 (13 Aug 2025)

https://packages.couchbase.com/clients/kafka/4.3.0/couchbase-kafka-connect-couchbase-4.3.0.zip[Download]
...
WARNING: Watch out for a change to the network detection heuristic.
If the connector is unable to connect to Couchbase Server after the upgrade, you might need to specify `couchbase.network=default` in your connector config.

=== Behavioral Changes

* https://jira.issues.couchbase.com/browse/JVMCBC-1660[JVMCBC-1660]:
The `auto` network selection heuristic has been changed to fall back to the `external` network if there is no exact address match and the `external` network is present.
+
Previously, if there was no exact match between an address in the connection string and an address in the cluster topology reported by the server, the connector would select the `default` network.
Now, if there is no match and an `external` network is present, the connector selects the `external` network.
+
TIP: If this change causes the connector to select the incorrect network for your deployment (you'll know because the connector will be unable to connect to the Couchbase Server cluster), specify `couchbase.network=default` in your connector config to force the connector to use the same network as before.

* https://jira.issues.couchbase.com/browse/KAFKAC-400[KAFKAC-400]:
*Source*
The default filter (`AllPassFilter`) now filters out documents whose IDs start with `_txn:`.
These documents contain Couchbase transaction metadata, and might not be JSON. Filtering them out prevents warnings like:
+
----
Skipping non-JSON document: bucket=<bucket> key=_default._default._txn:client-record
----

* https://jira.issues.couchbase.com/browse/KAFKAC-401[KAFKAC-401]:
*Source*
The default filter (`AllPassFilter`) now _actually_ filters out documents in the Couchbase system scope.
This feature was originally announced in connector version 4.2.4, but it didn't work until now.
...
* https://jira.issues.couchbase.com/browse/KAFKAC-383[KAFKAC-383]:
*Source*
The new `couchbase.jsonpath.filter` connector config property lets you filter out documents that do not match some https://github.com/json-path/JsonPath[JSONPath expression].

* https://jira.issues.couchbase.com/browse/KAFKAC-382[KAFKAC-382]: Certain connector config properties now support contextual overrides.
This lets you have a default value for a property, and override the default in different contexts.
+
In the source connector, the context is usually the collection a document came from.
In the sink connector, the context is usually the Kafka topic a record came from.
+
...
* https://jira.issues.couchbase.com/browse/KAFKAC-398[KAFKAC-398]:
*Source*
Metrics reported by the connector are now tagged with the UUID of the Couchbase cluster and the name of the bucket.

* https://jira.issues.couchbase.com/browse/KAFKAC-390[KAFKAC-390]:
Lifecycle milestone messages are now logged to a category suffixed by the milestone type.
For example, `SOURCE_OFFETS_READ` events are now logged to category `com.couchbase.connect.kafka.SourceDocumentLifecycle.SOURCE_OFFSETS_READ`.
This lets you filter the events you're interested in by configuring the log levels for the logging categories specific to each event type.

* https://jira.issues.couchbase.com/browse/KAFKAC-397[KAFKAC-397]:
The connector no longer depends on Apache Commons Lang.

* https://jira.issues.couchbase.com/browse/KAFKAC-402[KAFKAC-402]:
Upgraded Couchbase Java SDK from `3.8.0` to `3.9.0`

* https://jira.issues.couchbase.com/browse/KAFKAC-403[KAFKAC-403]:
Upgraded DCP client from `0.54.0` to `0.55.0`

[[v4.2.8]]
== Version 4.2.8 (22 Apr 2025)

https://packages.couchbase.com/clients/kafka/4.2.8/couchbase-kafka-connect-couchbase-4.2.8.zip[Download]

This maintenance release includes a few minor improvements.

=== Enhancements

* https://jira.issues.couchbase.com/browse/KAFKAC-377[KAFKAC-377]:
*Source* Document lifecycle log messages now include a `taskUuid` field that uniquely identifies the source task instance handling the message.

* https://jira.issues.couchbase.com/browse/KAFKAC-379[KAFKAC-379]:
*Source* `RawJsonWithMetadataSourceHandler` now has a protected method `customizeMetadata` that subclasses can override to add or remove metadata elements (like scope and collection, for example). Thank you to https://github.com/yeikel[Yeikel].

* https://jira.issues.couchbase.com/browse/KAFKAC-381[KAFKAC-381]:
Upgrade DCP client from `0.53.0` to `0.54.0`.

* https://jira.issues.couchbase.com/browse/KAFKAC-380[KAFKAC-380]:
Upgrade Couchbase Java SDK from `3.7.9` to `3.8.0`.


[[v4.2.7]]
== Version 4.2.7 (12 Mar 2025)

https://packages.couchbase.com/clients/kafka/4.2.7/couchbase-kafka-connect-couchbase-4.2.7.zip[Download]

This maintenance release includes an important fix for a potential resource leak, and a handful of minor improvements.

=== Enhancements

* https://jira.issues.couchbase.com/browse/KAFKAC-366[KAFKAC-366]:
*Source* Added an optional `onRecordCommitted` method to the `SourceHandler` interface.
This method is called whenever the Kafka Connect framework calls `SourceTask.commitRecord`.
Subclasses may override this method to do something special with committed record metadata beyond the usual document lifecycle logging.
The default implementation does nothing, which is always okay.
+
Thank you to new contributor Eyal Nussbaum!

* https://jira.issues.couchbase.com/browse/KAFKAC-369[KAFKAC-369]:
*Source* `OFFSET_COMMIT_HOOK` lifecycle messages are now logged at DEBUG level instead of INFO.

* https://jira.issues.couchbase.com/browse/KAFKAC-374[KAFKAC-374]:
Upgrade DCP client from `0.52.0` to `0.53.0`.

* https://jira.issues.couchbase.com/browse/KAFKAC-375[KAFKAC-375]:
Upgrade Couchbase Java SDK from `3.7.5` to `3.7.9`.

=== Bug Fixes

* https://jira.issues.couchbase.com/browse/KAFKAC-370[KAFKAC-370]:
*Source* Fixed a race condition that could cause a source task to fail to disconnect its DCP client if the task was stopped immediately after starting.
This could lead to a resource leak for the Kafka Connect worker process, and performance issues for Couchbase Server.
